...
🏢 [REF-COMPANY-03 — Texto "Sobre nós" administrável pelo Admin](REF-COMPANY-03-sobre-nos-editavel.md) — **Implementada; sem migration (zero risco de deploy)**. `SOBRE_TEXTO` (array hardcoded em `constants/storeInfo.js`) migra para `company_info.sobre` reaproveitando o RPC `set_company_info` já existente — que já aceita qualquer chave nova via merge raso, sem exigir SQL novo (exatamente o "crescer sem refatoração" previsto na REF-COMPANY-01). Representado como string única (parágrafos separados por linha em branco) para caber num `<textarea>` simples no Admin; `SobreScreen.jsx` faz o split na renderização. Gates: `test:company` + `test:company-guard` (novo invariante: `SOBRE_TEXTO` nunca reaparece).

...
**Fase de frontend (REF — arquitetura, fora da trilha NORM):** 🔒 [REF-APP-01 — Modularização do `App.jsx`](REF-APP-01-modularizacao-appjsx.md) — **DESENHO CONGELADO (sem execução)**: diagnóstico do monólito (3866 linhas → ~48 módulos), ordem em ondas, riscos, validação e estratégia zero-mudança-funcional. Pré-condição obrigatória (achado B1): 🟦 [REF-APP-01 · Onda 0 — deps.audit](REF-APP-01-onda-0-deps-audit.md) — **PROPOSTA (não aplicada)** para reestruturar a regra D do `test:deps` (importer rígido `['App.jsx']` → allowlist incremental), preservando A/B/C/E/F. **Execução só após Onda 0 aplicada + congelamento explícito.** Independente do NORM-06.

...
- **Status de implementação:** **Bloco F1 ENCERRADO e MERGEADO na `main`** (merge `f25e7cb`, 2026-06-30 — nova baseline oficial): **F1A (estrutura)** APLICADA em 2026-06-28 · **F1B (invariantes STI I1–I4)** APLICADA em 2026-06-29 · **F1B-Errata-01 (funções STI SECURITY DEFINER)** · **F1C = NORM-06.1 (RLS)** APLICADA em 2026-06-30 (tag F1A `norm-06-f1a-complete` → `4177a45`). **F2+ (backfill/DataService/Admin/UI) seguem pendentes** (F2 bloqueado pelas colisões abaixo). Evidências e ledgers: [F1A](NORM-06-F1A-execution-plan.md) · [F1B](NORM-06-F1B-execution-plan.md) · [NORM-06.1/F1C](NORM-06.1-execution-plan.md) · [MERGE-F1](MERGE-F1-execution-plan.md). A F1B ratificou **D-I4-ADIC** (I4 também bloqueia `business→collection` com adicionais referenciando) e endureceu o TOCTOU com `FOR SHARE`. **⚠️ Pré-condição descoberta para o F2:** o backfill colide em `unique_nome_categoria` (dois "Encanto Mineiro" em c4+c8; duas "Marmita G 2 Proteínas…" em c1+c2) — resolução humana obrigatória antes do F2 (ver F1B Execution Plan). *(A arquitetura deste ADR permanece imutável — apenas o status de implementação foi anotado.)*

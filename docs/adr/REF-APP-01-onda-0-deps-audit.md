...
- **Status:** ✅ **APLICADA (2026-06-30, commit `1b55379`)** — com o **ajuste do usuário**: a D2 passou a ser uma **regra estrutural por camada** (camadas fora de UI/presentation não importam lógica pura/domínio), em vez do "nenhum importer é domínio" da proposta original (que já é coberto por A/B). Diff exclusivamente em `tests/deps.audit.mjs`. Ver §7.
...
## 7. Aplicação (2026-06-30) — commit `1b55379`

Autorizada com **ajuste**: a regra D passou a ter **três checagens** (A/B/C/E/F **inalteradas**):

- **D1 (allowlist evolutiva):** importers de `pricing`/`addons` ⊆ `DOMAIN_CONSUMERS` (hoje só `App.jsx`; cresce commit-a-commit).
- **D2 (estrutural por camada — ajuste do usuário):** `const NON_UI_LAYER = /^(services|lib|data|constants)\//` não importa `PURE_LOGIC = [pricing, addons, format]`. Só UI/presentation (`components/`, `pages/`, `hooks/`, `App.jsx`) pode consumir lógica pura. **D2 é complementar à D1**: barra a violação de camada mesmo que o arquivo esteja na allowlist.
- **D3 (higiene):** allowlist sem entrada morta.

> **Nota de escopo da D2:** `format` é tratado como lógica pura protegida (não pode ser importado por `services/lib/data/constants`). `utils/` **não** está entre as camadas barradas — é folha compartilhada — então `utils/catalog.js → utils/format.js` (uso de `norm`) permanece válido. Os domínios de negócio (`pricing`/`addons`) seguem cobertos também pela D1 + A/B/C/E/F.

**Evidência:** `test:deps` verde (A,B,C, **D1×2, D2, D3**, E, F); `test:pricing`/`test:addons`/`build` verdes. Teste negativo (PoC isolado): D1 pega consumidor-surpresa; **D2 pega `services/`→`pricing` mesmo na allowlist** e `constants/`→`format`. Diff só em `tests/deps.audit.mjs` (+37/−3). **Rollback:** `git revert 1b55379`.

> ✅ **ONDA 0 — APLICADA.** Pré-condição B1 da REF-APP-01 resolvida. **Nenhuma extração iniciada** — aguardando o próximo comando do usuário para o congelamento da fase de execução.

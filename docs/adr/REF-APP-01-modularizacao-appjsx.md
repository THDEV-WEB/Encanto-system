...
**Status do aceite (2026-07-10, pós-Onda 9):** ✅ `App.jsx` = **124 linhas** (router `App` + `AppShell` + 3 imports-token estruturais + mapa de rastreio das extrações em comentário; **~30 linhas de código efetivo, zero lógica de domínio**) — atende ao AppRouter mínimo; os comentários de rastreio (Ondas 1–9), já espelhados no §10, podem ser condensados para um ponteiro do ADR num passo cosmético opcional. ✅ Suíte **7/7 verde**. ✅ INV-CK + fluxo de checkout preservados (`test:checkout` golden verde). ✅ Cada módulo rastreável a 1 commit.
...
15. **Onda 9 (NÚCLEO `StoreApp` — etapa final)** (`StoreApp` → `pages/StoreApp.jsx`) — ✅ **EXECUTADA (2026-07-10):** 9.1 extração byte-a-byte do `StoreApp` (`61010f7`; move-puro **provado por `diff` idêntico** entre corpo original e movido; o router `App` passa a importar de `pages/`; poda dos ~18 imports órfãos decorrentes; **allowlist D1** += `pages/StoreApp.jsx` [consome addons no mesmo commit]; **token Regra F addons** em `App.jsx`, espelhando o token pricing) · 9.2 poda de imports órfãos pré-existentes (`React` default/`useEffect`/`useRef`, resíduo da Onda 8) e documentação do `DS` como **token do guard `test:ds-micro` R2** (`c465579`; o guard R2 exige que `App.jsx` importe `DS`). 1 mudança lógica = 1 commit; suíte 7/7 + build verdes a cada passo; sem push; `App.jsx` 998→124 linhas (−874). App.jsx torna-se **orquestrador mínimo**: router `App` (store/login/admin) + `AppShell`, com **3 imports-token estruturais** (`precoVitrine`+`resolverAdicionais` → Regra F; `DS` → guard ds-micro R2) e **zero lógica de domínio**. INV-CK, checkout e domínios intocados. ✅
...
O desenho deste documento foi **congelado e ratificado**, e as pré-condições de execução — (a) **Onda 0 aplicada** (regra D reestruturada), (b) congelamento explícito da **fase de execução** pelo usuário, e (c) Execution Plan por ondas (padrão F1A/F1B) — foram **cumpridas**; a **execução está CONCLUÍDA** (Ondas 0–9 + rede R9 aplicadas; `App.jsx` reduzido a orquestrador mínimo — router/bootstrap sem lógica de domínio). O **F2 do NORM-06 permanece bloqueado** e independente desta fase. Melhorias arquiteturais adicionais (dedup/consolidação B4, `import './index.css'` → `main.jsx`) pertencem à **REF-APP-02** e não foram iniciadas.

> ✅ **REF-APP-01 — TECNICAMENTE CONCLUÍDA.** Desenho congelado/ratificado; Onda 0 + congelamento aplicados. **Ondas 0–9 + rede R9 executadas (2026-07-06→10).** `App.jsx` reduzido a **orquestrador mínimo** (router/bootstrap, sem lógica de domínio; `App.jsx` 3866→124 linhas). Consolidações/dedup (B4) e `index.css`→`main.jsx` = **REF-APP-02** (não iniciada).

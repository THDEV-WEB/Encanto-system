...
- **O campo e-mail vem pré-preenchido com o e-mail REAL do administrador de produção** (`<email-real-admin-encanto>` — redigido — hardcoded como valor inicial do `useState`). Mesmo risco que a auditoria da E2E-01 já havia identificado para o login do Admin ("nunca a conta real") — aqui fica confirmado que o próprio código expõe esse e-mail como valor padrão do formulário. **Nota (2026-08-24): já corrigido — `AdminLogin.jsx:8` hoje usa `useState('')` vazio.**

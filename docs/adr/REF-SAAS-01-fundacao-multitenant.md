...
- **Status:** 🟢 **Implementado — Ondas 0–6 (com subfases), 7.1 e 8 concluídas e em produção** (ver detalhamento por onda em [REF-SAAS-01-plano-ondas.md](../ref/REF-SAAS-01-plano-ondas.md)). Onda 7.2 (integração oficial Meta/BSP) permanece adiada por decisão do dono; Onda 9 (mobile) não é bloqueante. Este documento continua sendo a referência arquitetural permanente da fundação multi-tenant — o §0 "Ground truth" abaixo descreve o estado do sistema em 2026-08-07, **antes** da implementação começar, e sustenta o raciocínio das decisões tomadas; não deve ser lido como o estado atual do banco.

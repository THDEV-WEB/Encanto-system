...
**Status:** 🟢 Auditoria aprovada (2026-07-23; cenário de "conta virgem"/1º acesso ficou fora do escopo, por decisão do dono). Ondas 1-4 aplicadas (mecânica de login mock; sessão real; Minha Conta + Meus Pedidos vazio; checkout logado + vínculo + Meus Pedidos com pedido real + Fidelidade) — ver §Divisão em ondas e §Ajustes encontrados na execução.
...
## Ajustes encontrados na execução (Onda 4, 2026-07-23)

1. **`p_request_id` é coluna `uuid`, não texto livre.** `criarPedidoFixture()` (`support/fixture-order.js`, chama `create_order` direto, sem UI) inicialmente gerava uma string tipo `e2e-fixture-order-<timestamp>-<random>` — o Postgres rejeitava com `invalid input syntax for type uuid`. Corrigido com `crypto.randomUUID()` (Node nativo). Achado trivial, sem impacto de arquitetura.
2. **CRÍTICO — achado 9 da auditoria (concorrência entre arquivos que mutam o mesmo fixture) deixou de ser hipotético e virou reproduzível.** Com `checkout-logado.spec.js`, `meus-pedidos.spec.js` (parte 2) e `fidelidade.spec.js` todos escrevendo pedidos/fidelidade do MESMO cliente fixture, rodar os 3 arquivos juntos com o `workers` padrão (>1 localmente) fazia o `afterEach` de um arquivo (`limparPedidosDoFixture`, que apaga `loyalty_accounts`) às vezes apagar o estado que OUTRO arquivo tinha acabado de armar (ex.: `fidelidade.spec.js` armava `stamps=10` para testar o resgate, e um `afterEach` concorrente de outro arquivo apagava a linha antes da página carregar) — falha intermitente, confirmada reproduzindo em isolamento (passava sempre sozinho) vs. em conjunto (falhava ~1 em 2). Cada describe já se serializa CONSIGO MESMO (`mode:'serial'`), mas isso nunca protegeu contra outro ARQUIVO rodando ao mesmo tempo em outro worker. **Correção arquitetural** (não um workaround pontual): `e2e/playwright.config.js` passa a fixar `workers: 1` sempre (antes: `CI ? 1 : undefined`, ou seja, só CI era seguro; local usava múltiplos workers e podia flakar). Elimina a classe inteira de corrida sem exigir uma 2ª identidade de fixture por arquivo (a auditoria decidiu por 1 único cliente fixture, como o admin) e sem a complexidade de um 2º "project" do Playwright só para specs que mutam o fixture. Custo: suite local mais lenta (~2min para 49 specs, ainda aceitável); CI já rodava assim, sem mudança de comportamento lá.

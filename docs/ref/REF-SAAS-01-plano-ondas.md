...
- **Status:** 🟢 **Em execução — Ondas 0, 1, 2 e 3 concluídas (2026-08-07/08); Onda 4 em andamento — 4.1 e 4.2 concluídas (2026-08-08), 4.3 pendente.** O dono confirmou explicitamente que o padrão elevado de validação comportamental das Ondas 2/3 (teste positivo E negativo, isolamento entre lojas provado por comportamento, compatibilidade com o Cliente Zero) **é permanente para toda onda futura**, com atenção redobrada nesta por ser a maior do roadmap.
...
| **4** | RLS + RPCs de pedidos/fidelidade/entrega/horário (`orders` + `delivery_fee`/`maquininha_fee`, `loyalty_*`, `business_hours_schedule`, `delivery_fee_config` → `store_settings`) — dividida em 3 subfases | Ondas 1–3 | 🔧 **Em execução — 4.1 ✅ e 4.2 ✅ concluídas (2026-08-08), 4.3 pendente** — ver seção própria abaixo |

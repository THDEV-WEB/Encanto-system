/* components/menu/SideDrawer.jsx — drawer lateral da loja (LOGIN-ARCH-02). Guest-first.
   Topo: entrar/conta · Fidelidade · Contato · Sobre · Termos · redes sociais. */
import { useAuth } from '../../hooks/useAuth.js';
import { STORE_INFO } from '../../constants/storeInfo.js';

const overlay = { position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 3100, display: 'flex', justifyContent: 'flex-end' };
const drawer = { width: '86%', maxWidth: 340, height: '100%', background: 'var(--white)', boxShadow: '-8px 0 30px rgba(0,0,0,.2)', display: 'flex', flexDirection: 'column', overflowY: 'auto' };
  ...
  return (
    <div style={overlay} onClick={e => e.target === e.currentTarget && onClose()}>
      <aside style={drawer}>
        ...
        <button style={item} onClick={() => onNavigate('sobre')}><span style={{ fontSize: 18 }}>ℹ️</span> Sobre nós</button>
        <button style={item} onClick={() => onNavigate('termos')}><span style={{ fontSize: 18 }}>📄</span> Termos e Políticas</button>

        <div style={{ display: 'flex', gap: 14, padding: 20, marginTop: 'auto' }}>

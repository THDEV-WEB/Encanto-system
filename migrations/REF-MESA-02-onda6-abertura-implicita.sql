-- ============================================================================
-- REF-MESA-02 · Onda 6 — Abertura implícita da sessão de mesa
-- ----------------------------------------------------------------------------
-- Primeiro pedido válido de uma mesa abre (ou reaproveita) a sessão
-- automaticamente -- sem exigir que o garçom aperte "Abrir sessão" antes.
-- Justificativa (pedida explicitamente pela autorização): reduz operação
-- manual, evita sessão aberta sem consumo, reduz erro operacional, e mantém
-- a transação do primeiro pedido como ponto confiável de criação (mesma
-- transação, sem janela de inconsistência entre "abrir" e "primeiro pedido").
--
-- Nova capability opt-in por loja: mesa_sessao_habilitada (independente de
-- mesa_habilitada/canal_qr/canal_admin) -- enquanto desligada (default),
-- tipo_pedido='mesa' se comporta EXATAMENTE como hoje (mesa_session_id fica
-- sempre NULL). Mesmo padrão de todas as capabilities anteriores deste
-- domínio -- nunca muda comportamento de loja que não optou.
--
-- _get_or_open_mesa_session(): função interna (sem GRANT a ninguém) chamada
-- de dentro de create_order(). Resolve a corrida real "dois clientes pedindo
-- na mesma mesa ao mesmo tempo, nenhuma sessão aberta ainda":
--   1. tenta achar sessão já aberta (caminho feliz, sem lock ainda);
--   2. se achou, adquire SELECT...FOR UPDATE na linha (serializa contra um
--      fechamento concorrente -- RPC de fechar_conta_mesa, onda futura, vai
--      adquirir o MESMO lock antes de mudar o status); reconfirma que ainda
--      está aberta sob o lock (pode ter fechado entre o passo 1 e o lock);
--   3. se não achou (ou fechou entre os passos), cria uma sessão nova +
--      tenta anexar a mesa -- se OUTRA transação venceu a corrida no meio
--      do caminho (unique_violation no índice parcial da Onda 2), descarta a
--      sessão órfã que acabou de criar (nada mais referencia ela ainda) e
--      usa a que venceu, sob lock.
-- Nunca usa espera passiva/retry em loop -- o próprio unique_violation do
-- Postgres é o mecanismo de arbitragem, sem corrida de leitura-depois-escrita
-- sem proteção.
--
-- create_order(): quando tipo_pedido='mesa' E mesa_sessao_habilitada=true E
-- origem_pedido IN ('qr_mesa','admin_garcom') (os 2 canais reais de mesa --
-- 'storefront'+'mesa' é uma combinação hoje tecnicamente aceita mas sem
-- canal correspondente, mantida sem sessão, comportamento intocado),
-- resolve/abre a sessão e grava orders.mesa_session_id. Nenhuma mudança de
-- payload exigida do client -- resolução inteiramente server-side.
--
-- Testes: scripts/mesa-02-onda6-abertura-implicita-test.mjs (E2E) -- inclui
-- teste de concorrência REAL com 2 conexões Postgres simultâneas (SAVEPOINT
-- não prova lock real -- FOR UPDATE só bloqueia entre transações
-- DIFERENTES). Reaproveita como referência os precedentes reais já
-- confirmados no repo: redeem_reward() (REF-LOYALTY-01) e
-- enc_claim_notifications() (REF-ORDER-01, FOR UPDATE SKIP LOCKED).
-- Rollback: REF-MESA-02-onda6-abertura-implicita-rollback.sql.
-- ============================================================================

BEGIN;

-- get_mesa_config: corpo apenas, RETURNS jsonb inalterado -- CREATE OR REPLACE simples.
...
AS $function$
  SELECT jsonb_build_object(
    ...
    'sessao_habilitada', COALESCE((SELECT valor FROM public.store_settings WHERE store_id = p_store_id AND chave = 'mesa_sessao_habilitada'), 'false') <> 'false'
  );
$function$;

-- set_mesa_config: ganha o 4o parametro booleano -- assinatura muda (novo parametro antes do
-- DEFAULT), exige DROP. Sem consumidor de UI ainda (gap ja registrado desde a REF-MESA-01), zero
-- blast radius no frontend.
DROP FUNCTION IF EXISTS public.set_mesa_config(boolean, boolean, boolean, uuid);

CREATE OR REPLACE FUNCTION public.set_mesa_config(p_habilitada boolean, p_canal_qr boolean, p_canal_admin boolean, p_sessao_habilitada boolean, p_store_id uuid DEFAULT default_store_id())
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public'
...
begin
  ...
  insert into public.store_settings (store_id, chave, valor) values (p_store_id, 'mesa_canal_admin', case when p_canal_admin then 'true' else 'false' end)
    on conflict (store_id, chave) do update set valor = excluded.valor;
  insert into public.store_settings (store_id, chave, valor) values (p_store_id, 'mesa_sessao_habilitada', case when p_sessao_habilitada then 'true' else 'false' end)
    on conflict (store_id, chave) do update set valor = excluded.valor;

  return jsonb_build_object('ok', true, 'habilitada', p_habilitada, 'canal_qr', p_canal_qr, 'canal_admin', p_canal_admin, 'sessao_habilitada', p_sessao_habilitada);
end;
$function$;
-- DROP FUNCTION apaga grants -- restaura exatamente o que existia (confirmado ao vivo antes desta
-- migration: postgres+authenticated+service_role, NUNCA anon -- escrita e sempre gated por
-- is_admin_of dentro do corpo, mas o GRANT em si ja era mais restrito que get_mesa_config).
-- Supabase concede EXECUTE a PUBLIC por padrao em toda funcao NOVA do schema public (achado ja
-- registrado pela REF-SEC-02) -- DROP+CREATE cria um objeto novo, entao precisa REVOKE explicito de
-- novo (nao so GRANT ao papel certo), mesmo padrao ja usado na migration original da Onda 1.
REVOKE EXECUTE ON FUNCTION public.set_mesa_config(boolean, boolean, boolean, boolean, uuid) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.set_mesa_config(boolean, boolean, boolean, boolean, uuid) FROM anon;
GRANT EXECUTE ON FUNCTION public.set_mesa_config(boolean, boolean, boolean, boolean, uuid) TO authenticated;

CREATE OR REPLACE FUNCTION public._get_or_open_mesa_session(
  p_store_id uuid, p_mesa_identificador text, p_origem_abertura text, p_opened_by_admin_user_id uuid
) RETURNS uuid
 LANGUAGE plpgsql
 SET search_path TO 'pg_catalog', 'public'
AS $function$
...
BEGIN
  SELECT mesa_session_id INTO v_session_id
  FROM public.mesa_session_mesas
  WHERE store_id = p_store_id AND mesa_identificador = p_mesa_identificador AND status_sessao = 'aberta'
  LIMIT 1;

  IF v_session_id IS NOT NULL THEN
    PERFORM 1 FROM public.mesa_sessions WHERE id = v_session_id FOR UPDATE;
    IF EXISTS (SELECT 1 FROM public.mesa_sessions WHERE id = v_session_id AND status = 'aberta') THEN
      RETURN v_session_id;
    END IF;
    -- fechou entre a leitura sem lock e o lock -- trata como se nao tivesse achado nada.
    v_session_id := NULL;
  END IF;

  INSERT INTO public.mesa_sessions (store_id, origem_abertura, opened_by_admin_user_id)
  VALUES (p_store_id, p_origem_abertura, p_opened_by_admin_user_id)
  RETURNING id INTO v_session_id;

  BEGIN
    INSERT INTO public.mesa_session_mesas (mesa_session_id, store_id, mesa_identificador)
    VALUES (v_session_id, p_store_id, p_mesa_identificador);
  EXCEPTION WHEN unique_violation THEN
    -- outra transacao venceu a corrida entre nosso SELECT inicial e este INSERT -- descarta a
    -- sessao orfa (nada a referencia ainda) e usa a que venceu, sob lock.
    DELETE FROM public.mesa_sessions WHERE id = v_session_id;
    SELECT mesa_session_id INTO v_session_id
    FROM public.mesa_session_mesas
    WHERE store_id = p_store_id AND mesa_identificador = p_mesa_identificador AND status_sessao = 'aberta'
    LIMIT 1;
...
END;
$function$;
REVOKE ALL ON FUNCTION public._get_or_open_mesa_session(uuid, text, text, uuid) FROM PUBLIC, anon, authenticated;

-- ── create_order(): resolve mesa_session_id quando a loja tiver mesa_sessao_habilitada ──────────

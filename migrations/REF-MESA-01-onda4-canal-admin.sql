-- ============================================================================
-- REF-MESA-01 · Onda 4 — Canal Admin/Garçom (criação manual de pedido de mesa)
-- ----------------------------------------------------------------------------
-- Fecha a validação server-side do 3º canal previsto desde a Onda 1
-- (orders_origem_pedido_valid já incluía 'admin_garcom'): quando
-- origem_pedido='admin_garcom', exige (a) mesa_canal_admin=true (capacidade
-- SEPARADA de mesa_habilitada/mesa_canal_qr, mesmo v_mesa_cfg já buscado) e,
-- diferente do canal QR (voltado ao cliente final, guest ou logado), (b) que o
-- CHAMADOR seja um admin autenticado DAQUELA loja (is_admin_of(v_store_id)) —
-- sem isso, qualquer `authenticated` (não só quem opera a loja) poderia se
-- passar por garçom e lançar pedidos em nome de qualquer loja com o canal
-- ligado. `storefront`/`qr_mesa` continuam sem essa exigência (são os canais
-- do cliente final).
--
-- create_order() continua sendo o ÚNICO mecanismo de persistência de pedido —
-- a tela nova do Admin (frontend desta Onda) chama a MESMA RPC, só variando
-- p_order.origem_pedido/tipo_pedido/mesa_identificador. Nenhum insert direto
-- em orders fora desta função, em lugar nenhum.
--
-- Idempotente (CREATE OR REPLACE). Preserva 100% o comportamento de
-- Delivery/Retirada/Mesa via storefront normal e via QR (Ondas 1-3).
-- ============================================================================
...
begin
  ...
  -- (nao so' quem opera a loja) poderia se passar por garcom.
  ...
  exception
    when unique_violation then
      ...
      begin insert into public.application_logs(module,operation,entity,entity_id,request_id,rpc,version,duration_ms,level,message,payload,sqlstate,context,origin)
        values('orders','create_order','order',null,p_request_id,'create_order','mesa-01-onda4',v_dur,'error',v_err,v_log,v_state,'unique_violation',current_user);
      exception when others then null; end;
      return jsonb_build_object('ok', false, 'error', v_err, 'sqlstate', v_state);
    when others then
      v_err := sqlerrm; v_state := sqlstate; v_dur := extract(epoch from clock_timestamp()-v_t0)*1000;
      begin insert into public.application_logs(module,operation,entity,entity_id,request_id,rpc,version,duration_ms,level,message,payload,sqlstate,context,origin)
        values('orders','create_order','order',null,p_request_id,'create_order','mesa-01-onda4',v_dur,'error',v_err,v_log,v_state,'create_order',current_user);
      exception when others then null; end;
      return jsonb_build_object('ok', false, 'error', v_err, 'sqlstate', v_state);
  end;
end;$function$;

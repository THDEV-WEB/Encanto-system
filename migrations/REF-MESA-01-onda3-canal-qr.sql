-- ============================================================================
-- REF-MESA-01 · Onda 3 — Canal QR (identificação de mesa via link/QR Code)
-- ----------------------------------------------------------------------------
-- Ideia central (reaproveita 100% da arquitetura já existente, zero infraestrutura
-- nova de resolução de loja): o storefront já é resolvido por domínio por-tenant
-- (resolve_store_from_origin/get_store_by_domain — cada loja tem seu próprio
-- subdomínio). Um QR Code de mesa aponta pro MESMO subdomínio da loja, só com um
-- parâmetro a mais na URL (`?mesa=07`) — nunca um store_id, então não há NADA de
-- tenant no link para um cliente adulterar (zero superfície de cross-tenant via QR,
-- por construção, não por validação adicional).
--
-- Esta migration cobre só a METADE que fica no servidor: create_order() já aceita
-- `p_order.origem_pedido` desde a Onda 1 (constraint `orders_origem_pedido_valid`
-- já inclui 'qr_mesa'), mas nunca validava a CAPACIDADE do canal em si — uma loja
-- com `mesa_habilitada=true` mas `mesa_canal_qr=false` aceitava um pedido marcado
-- como vindo do QR mesmo sem esse canal estar ligado. Fecha esse gap: reaproveita
-- o MESMO `v_mesa_cfg` já buscado (get_mesa_config, uma única chamada), mesmo
-- padrão fail-closed/mensagem genérica já usado pra `mesa_habilitada`.
--
-- Front-end correspondente (já commitado, Onda 3 parte 1): hooks/useMesaFromQuery.js
-- lê `?mesa=` da URL no boot da loja e só aplica automaticamente (deliveryMode=
-- 'mesa' + mesaIdentificador + origemPedido='qr_mesa') quando mesaConfig.canal_qr
-- já veio true do servidor — o client nunca decide sozinho, só reflete o que o
-- servidor autorizou; a validação de verdade é a desta migration.
--
-- Geração de imagem/impressão do QR Code fica FORA desta onda (puramente visual/
-- operacional, não é o que a REF pediu para provar arquiteturalmente) — só o
-- link/parâmetro e a validação server-side, que é a parte que importa.
--
-- Idempotente (CREATE OR REPLACE). Preserva 100% dos pedidos existentes e o
-- comportamento de Delivery/Retirada/Mesa via storefront normal (Ondas 1/2).
-- ============================================================================
...
begin
  ...
  -- REF-MESA-01 · Onda 3: canal QR e' uma capacidade SEPARADA (mesa_canal_qr) -- uma loja pode ter
  -- Mesa habilitada mas SEM aceitar pedidos vindos de QR (ex.: so' quer o garcom lancando). Reusa o
  -- MESMO v_mesa_cfg (uma unica chamada a get_mesa_config), mesmo padrao fail-closed/mensagem
  -- generica anti-enumeracao das checagens de tenant acima.
  ...
  elsif v_origem_pedido = 'qr_mesa' then
    -- origem_pedido='qr_mesa' sem tipo_pedido='mesa' nao faz sentido (o canal QR so existe pra
    -- identificar mesa) -- trata como combinacao invalida, mesma mensagem generica.
    return jsonb_build_object('ok', false, 'error', 'canal indisponivel para esta loja');
  end if;
  ...
  exception
    when unique_violation then
      ...
      begin insert into public.application_logs(module,operation,entity,entity_id,request_id,rpc,version,duration_ms,level,message,payload,sqlstate,context,origin)
        values('orders','create_order','order',null,p_request_id,'create_order','mesa-01-onda3',v_dur,'error',v_err,v_log,v_state,'unique_violation',current_user);
      exception when others then null; end;
      return jsonb_build_object('ok', false, 'error', v_err, 'sqlstate', v_state);
    when others then
      v_err := sqlerrm; v_state := sqlstate; v_dur := extract(epoch from clock_timestamp()-v_t0)*1000;
      begin insert into public.application_logs(module,operation,entity,entity_id,request_id,rpc,version,duration_ms,level,message,payload,sqlstate,context,origin)
        values('orders','create_order','order',null,p_request_id,'create_order','mesa-01-onda3',v_dur,'error',v_err,v_log,v_state,'create_order',current_user);
      exception when others then null; end;
      return jsonb_build_object('ok', false, 'error', v_err, 'sqlstate', v_state);
  end;
end;$function$;

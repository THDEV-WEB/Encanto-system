-- ============================================================================
-- REF-MESA-02 · Onda 19 — nome/telefone do cliente viram OPCIONAIS no canal admin_garcom
-- ----------------------------------------------------------------------------
-- Achado do dono: pedir telefone de quem so esta sentado numa mesa fisica (atendimento com garcom)
-- soa estranho -- diferente do autoatendimento via QR, onde o proprio cliente digita os dados pra
-- si mesmo (esse canal, qr_mesa, e tambem o storefront tradicional continuam exigindo os dois, sem
-- nenhuma mudanca). So o canal admin_garcom (NovoPedidoMesaModal.jsx, garcom lancando manualmente)
-- fica com os 2 campos opcionais -- a identificacao primaria da comanda de mesa sempre foi (e
-- continua sendo) o NUMERO DA MESA, ja obrigatorio.
--
-- Sem nome informado: create_order() usa 'Mesa <numero>' como nome. Sem telefone informado: gera um
-- placeholder UNICO por pedido (prefixo 'sem-telefone-' + uuid aleatorio) -- nunca um numero de
-- verdade (normalize_phone() so produz digitos, entao esse placeholder jamais colide com um
-- telefone real) e nunca reaproveitado entre pedidos diferentes (evita o upsert por (store_id,phone)
-- misturar/"herdar" nome de um cliente anonimo diferente -- cada mesa sem telefone vira seu proprio
-- registro em customers). Frontend (utils/format.js::telefoneExibivel) esconde esse placeholder de
-- qualquer tela que mostra telefone pro humano (AdminPedidos/AdminDashboard/comanda impressa).
--
-- Testes: scripts/mesa-02-onda19-nome-telefone-opcional-test.mjs (E2E).
-- Rollback: REF-MESA-02-onda19-nome-telefone-opcional-garcom-rollback.sql.
-- ============================================================================
...
begin
  ...
  begin
    ...
    -- REF-MESA-02 · Onda 19 (dono, 2026-09-11): no canal admin_garcom (garcom lancando manualmente
    -- pela mesa), nome/telefone do cliente viram OPCIONAIS -- pedir telefone de quem so esta sentado
    -- numa mesa fisica soa estranho pro garcom, diferente do autoatendimento via QR (onde o proprio
    -- cliente digita pra si mesmo -- storefront/qr_mesa continuam exigindo os dois, sem mudanca). A
    -- identificacao primaria da comanda continua sendo o NUMERO DA MESA (sempre obrigatorio, checado
    -- logo abaixo). Sem nome informado: cai no nome da propria mesa. Sem telefone informado: gera um
    -- placeholder UNICO por pedido (normalize_phone() so produz digitos, entao um placeholder com
    -- letras nunca colide com um numero de verdade) -- cada pedido sem telefone vira seu PROPRIO
    -- cliente anonimo no upsert por (store_id,phone) logo abaixo, nunca reaproveita/mistura
    -- identidade com outro pedido (evitaria um pedido antigo "herdar" retroativamente o nome de um
    -- pedido novo feito depois na mesma mesa).
    if v_name is null then
      if v_tipo_pedido = 'mesa' and v_origem_pedido = 'admin_garcom' then
        v_name := 'Mesa ' || coalesce(v_mesa_identificador, '?');
      else
        raise exception 'name do cliente e obrigatorio';
      end if;
    end if;
    if v_phone is null then
      if v_tipo_pedido = 'mesa' and v_origem_pedido = 'admin_garcom' then
        v_phone := 'sem-telefone-' || replace(gen_random_uuid()::text, '-', '');
      else

-- ============================================================================
-- REF-LOYALTY-02 · Onda 2 — Integração do resgate ao fluxo real de criação do pedido
-- ----------------------------------------------------------------------------
-- Decisões de negócio aprovadas pelo dono (ver docs/ref/REF-LOYALTY-02-auditoria.md):
--   1) meta atingida != resgate -- a 10a compra so' libera; a compra QUE PEDE pra usar a
--      recompensa e' que recebe o desconto (11a no exemplo).
--   2) desconto incide SOMENTE sobre o subtotal de PRODUTOS -- nunca sobre delivery_fee/
--      maquininha_fee/adicional_pagamento_fee.
--   3) percentual vem de store_settings (ja configuravel desde REF-LOYALTY-AUDIT-01), nunca
--      hardcoded.
--   4) resgate passa a ocorrer no CONTEXTO de um pedido real -- fica vinculado a ele
--      (order_id, ja suportado desde a Onda 1 desta REF).
--   5) reset (stamps -= required) so' acontece no resgate EFETIVO, nunca so' por atingir a meta
--      (o nucleo ja fazia isso desde REF-LOYALTY-01 -- nao mudou).
--   8) create_order/desconto/resgate precisam ser UMA UNICA transacao atomica.
--
-- MECANISMO (reaproveita o padrao ja existente de _resolve_item_pricing/_resolve_delivery_fee --
-- nenhuma arquitetura nova, conforme instrucao explicita do dono):
--   - NENHUMA mudanca de assinatura em create_order() -- o pedido de uso da recompensa viaja
--     como mais um campo OPCIONAL dentro do MESMO p_order jsonb (`usar_recompensa_fidelidade`),
--     exatamente como tipo_pedido/mesa_identificador/status ja viajam. O frontend so' declara a
--     INTENCAO; o servidor recalcula elegibilidade/percentual/valor do zero, nunca confia no
--     que o client mandar (mesma filosofia ADVISORY de delivery_fee/maquininha_fee).
--   - Nova funcao interna `_redeem_loyalty_for_order` (mesmo padrao/grants de
--     _resolve_item_pricing/_resolve_delivery_fee/_get_or_open_mesa_session: SECURITY DEFINER,
--     REVOKE ALL de PUBLIC/anon/authenticated -- so' create_order/redeem_reward chamam) concentra
--     a checagem-com-lock (`FOR UPDATE`) + debito + insercao no ledger. redeem_reward() (RPC
--     publica, resgate manual/admin) e' REFATORADA pra delegar a MESMA funcao -- elimina
--     duplicacao da logica de debito entre os 2 caminhos (manual e automatico).
--   - Dentro de create_order: a elegibilidade e' checada (com o lock `FOR UPDATE` em
--     loyalty_accounts, tomado o quanto antes -- mesma tecnica ja usada em redeem_reward hoje)
--     LOGO apos resolver o cliente, ANTES de qualquer INSERT -- se `usar_recompensa_fidelidade`
--     foi pedido e NAO ha elegibilidade real, o pedido inteiro e recusado (fail-closed, mesmo
--     padrao ja usado pela divergencia de delivery_fee/maquininha_fee: reapresenta e pede nova
--     confirmacao, nunca cria um pedido que cobra diferente do que o cliente viu na tela).
--   - O debito EFETIVO (stamps -= required, evento 'redeemed' com order_id) so' acontece DEPOIS
--     que orders/order_items ja foram inseridos (precisa do order_id) -- mas ainda dentro da
--     MESMA transacao, e NAO dentro de um bloco best-effort: se o debito falhar por qualquer
--     motivo (nao deveria, o lock ja garantiu elegibilidade), a excecao propaga pro handler
--     externo e TODA a transacao (pedido incluso) roda pra tras -- nunca fica desconto aplicado
--     sem o debito correspondente.
--   - MUTUAMENTE EXCLUSIVO dentro do mesmo pedido: se este pedido RESGATOU a recompensa, ele
--     NAO tambem GANHA um novo selo pra si (loyalty_grant nao e chamado nesse caso) -- o novo
--     ciclo comeca so' no PROXIMO pedido, exatamente como o exemplo aprovado pelo dono descreve
--     (compra 11 reseta pra 0; a compra 12 e' que e' "1" do novo ciclo).
--
-- NUCLEO INTOCADO: validacao de mesa/canal, _resolve_delivery_fee, _resolve_item_pricing,
-- idempotencia por request_id, o indice unico parcial de loyalty_events, o trigger de
-- cancelamento -- nada disso muda. Unica reordenacao: o INSERT em customers passa a acontecer
-- um pouco mais cedo (antes de resolver a taxa de entrega, em vez de logo antes do INSERT em
-- orders) -- necessario para ter v_customer_id disponivel na hora de checar elegibilidade de
-- fidelidade; nenhuma mudanca de comportamento do upsert em si (mesmo ON CONFLICT, mesmos
-- campos).
--
-- Idempotente (CREATE OR REPLACE, mesma assinatura de create_order/redeem_reward -- nao cria
-- overload, licao da Onda 1 desta mesma REF). Testado no projeto E2E (bgzcrovskjbktdxkhemd)
-- antes de qualquer cogitacao de producao -- ver scripts/loyalty-02-onda2-test.mjs.
...
-- ─────────────────────────────────────────────────────────────────────────
-- 1) _redeem_loyalty_for_order — nucleo do debito, reaproveitado por create_order (automatico)
--    e por redeem_reward (manual/admin, refatorada abaixo pra delegar aqui). Assume que quem
--    chama JA resolveu p_customer_id/p_store_id com autoridade (create_order resolve pelo
--    proprio cliente que fez o pedido; redeem_reward resolve por auth.uid()/is_admin_of, como
--    sempre fez) -- esta funcao so' garante elegibilidade-com-lock + debito + ledger, uma unica
--    vez, corretamente, nos 2 lugares.
-- ─────────────────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public._redeem_loyalty_for_order(
  p_customer_id uuid,
  p_store_id uuid,
  p_order_id uuid DEFAULT NULL::uuid,
  p_subtotal_itens numeric DEFAULT NULL::numeric,
  p_origem text DEFAULT 'redeem'
)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  update public.loyalty_accounts
   ...
   where customer_id = p_customer_id returning stamps into v_stamps;
  insert into public.loyalty_events (customer_id, order_id, tipo, delta, stamps_after, origem, note, store_id, discount_pct, discount_amount)
    values (p_customer_id, p_order_id, 'redeemed', -v_required, v_stamps, p_origem,
            'recompensa ' || v_discount || '%', p_store_id, v_discount, v_discount_amount);

  return jsonb_build_object('ok', true, 'stamps', v_stamps, 'required', v_required, 'discount', v_discount,
                            'discount_amount', v_discount_amount,
                            'rewards_redeemed', (select rewards_redeemed from public.loyalty_accounts where customer_id = p_customer_id));
end;
$function$;

REVOKE ALL ON FUNCTION public._redeem_loyalty_for_order(uuid, uuid, uuid, numeric, text) FROM PUBLIC, anon, authenticated;

-- ─────────────────────────────────────────────────────────────────────────
-- 2) redeem_reward — mesma assinatura da Onda 1 (identidade/permissao 100% inalteradas: ramo
--    admin via is_admin_of, ramo cliente via auth.uid(), mesmas mensagens de erro). So' a etapa
--    final (debito+ledger) passa a delegar pra _redeem_loyalty_for_order, em vez de duplicar a
--    logica -- 0 mudanca de comportamento observavel (mesmos testes da Onda 1 continuam
--    validos, reexecutados nesta onda).
-- ─────────────────────────────────────────────────────────────────────────
...
begin
  ...
  return public._redeem_loyalty_for_order(v_cid, v_store, p_order_id, p_subtotal_itens,
                                          case when v_admin then 'admin' else 'redeem' end);
end;
$function$;
...
-- ─────────────────────────────────────────────────────────────────────────
-- 3) create_order — mesma assinatura (5 args, inalterada). `usar_recompensa_fidelidade` (novo
--    campo OPCIONAL dentro de p_order, default false quando ausente) integra o resgate.
-- ─────────────────────────────────────────────────────────────────────────
  ...
  exception
    when unique_violation then
      ...
      begin insert into public.application_logs(module,operation,entity,entity_id,request_id,rpc,version,duration_ms,level,message,payload,sqlstate,context,origin)
        values('orders','create_order','order',null,p_request_id,'create_order','loyalty-02-onda2',v_dur,'error',v_err,v_log,v_state,'unique_violation',current_user);
      exception when others then null; end;
      return jsonb_build_object('ok', false, 'error', v_err, 'sqlstate', v_state);
    when others then
      v_err := sqlerrm; v_state := sqlstate; v_dur := extract(epoch from clock_timestamp()-v_t0)*1000;
      begin insert into public.application_logs(module,operation,entity,entity_id,request_id,rpc,version,duration_ms,level,message,payload,sqlstate,context,origin)
        values('orders','create_order','order',null,p_request_id,'create_order','loyalty-02-onda2',v_dur,'error',v_err,v_log,v_state,'create_order',current_user);
      exception when others then null; end;
      return jsonb_build_object('ok', false, 'error', v_err, 'sqlstate', v_state);
  end;
end;$function$;

-- CREATE OR REPLACE preserva os grants existentes de create_order (mesma assinatura de sempre,
-- 5 args -- nao muda o OID da funcao, diferente do que aconteceu com redeem_reward na Onda 1
-- desta REF quando um parametro foi ACRESCENTADO; aqui nenhum parametro novo foi adicionado).

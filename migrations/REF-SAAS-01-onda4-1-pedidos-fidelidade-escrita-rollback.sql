-- REF-SAAS-01 · Onda 4.1 — Rollback: restaura policies (is_admin() cego, sem correlacao de loja),
-- orders_health/create_order as assinaturas antigas, triggers/RPCs sem store_id, store_id
-- nullable/sem DEFAULT nas 6 tabelas.

BEGIN;

-- ===== RLS de volta ao estado pre-Onda-4.1 =====
ALTER POLICY "notification_outbox_admin_read" ON public.notification_outbox USING (public.is_admin());

ALTER POLICY "loyalty_events_admin_all" ON public.loyalty_events USING (public.is_admin()) WITH CHECK (public.is_admin());
...
CREATE POLICY "loyalty_events_read_own" ON public.loyalty_events
  ...
  USING (public.is_admin() OR (customer_id IN (SELECT customers.id FROM public.customers WHERE customers.auth_user_id = auth.uid())));

ALTER POLICY "loyalty_accounts_admin_all" ON public.loyalty_accounts USING (public.is_admin()) WITH CHECK (public.is_admin());
...
CREATE POLICY "order_events_read_own" ON public.order_events
  FOR SELECT TO authenticated
  USING (public.is_admin() OR (order_id IN (SELECT o.id FROM public.orders o JOIN public.customers c ON c.id = o.customer_id WHERE c.auth_user_id = auth.uid())));

ALTER POLICY "Admin all order_items" ON public.order_items USING (public.is_admin()) WITH CHECK (public.is_admin());
...
CREATE POLICY "Cliente le proprios order_items" ON public.order_items
  FOR SELECT TO authenticated
  USING (order_id IN (SELECT o.id FROM public.orders o JOIN public.customers c ON c.id = o.customer_id WHERE c.auth_user_id = auth.uid()));

ALTER POLICY "Admin all orders" ON public.orders USING (public.is_admin()) WITH CHECK (public.is_admin());
...
-- ===== orders_health: volta a 0 argumentos =====
DROP FUNCTION IF EXISTS public.orders_health(uuid);
...
begin
  if new.payment_method is distinct from old.payment_method then
    insert into public.order_events(order_id,tipo,usuario,payload)
      values(new.id,'PAGAMENTO_ALTERADO',current_user,jsonb_build_object('antes',old.payment_method,'depois',new.payment_method));
  end if;
  if new.address is distinct from old.address then
    insert into public.order_events(order_id,tipo,usuario,payload)
      values(new.id,'ENDERECO_ALTERADO',current_user,jsonb_build_object('antes',old.address,'depois',new.address));
  end if;
  if new.observacoes is distinct from old.observacoes then
    insert into public.order_events(order_id,tipo,usuario,payload)
      values(new.id,'OBS_ALTERADA',current_user,jsonb_build_object('antes',old.observacoes,'depois',new.observacoes));
  end if;
  if new.total is distinct from old.total then
    insert into public.order_events(order_id,tipo,usuario,payload)
      values(new.id,'PEDIDO_EDITADO',current_user,jsonb_build_object('campo','total','antes',old.total,'depois',new.total));
  end if;
  return null;
end;$function$;
...
begin
  if tg_op = 'INSERT' then
    insert into public.order_events(order_id, tipo, status_anterior, status_novo)
      values (new.id, 'PEDIDO_CRIADO', null, new.status);
...
-- ===== create_order: volta a assinatura de 4 argumentos (drop a de 5 antes) =====
DROP FUNCTION IF EXISTS public.create_order(jsonb, jsonb, jsonb, uuid, uuid);
...
ALTER TABLE public.loyalty_accounts     ALTER COLUMN store_id DROP NOT NULL;
ALTER TABLE public.order_events         ALTER COLUMN store_id DROP NOT NULL;
ALTER TABLE public.order_items          ALTER COLUMN store_id DROP NOT NULL;
ALTER TABLE public.orders               ALTER COLUMN store_id DROP NOT NULL;

ALTER TABLE public.notification_outbox  ALTER COLUMN store_id DROP DEFAULT;
ALTER TABLE public.loyalty_events       ALTER COLUMN store_id DROP DEFAULT;
ALTER TABLE public.loyalty_accounts     ALTER COLUMN store_id DROP DEFAULT;
ALTER TABLE public.order_events         ALTER COLUMN store_id DROP DEFAULT;
ALTER TABLE public.order_items          ALTER COLUMN store_id DROP DEFAULT;

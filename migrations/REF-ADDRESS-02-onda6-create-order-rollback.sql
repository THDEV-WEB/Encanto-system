-- Rollback ADR REF-ADDRESS-02 · Onda 6
-- Restaura create_order EXATAMENTE ao corpo capturado por introspecção antes desta onda (Onda 1/6,
-- confirmado idêntico nas duas capturas). NÃO remove orders.endereco_id/FK (pré-existentes a esta
-- migration, não criados por ela — ver Onda 1). Pedidos já criados com endereco_id preenchido
-- continuam com o valor gravado (rollback é só da função, não um DELETE/UPDATE de dados).

-- REF-PROD-GOLIVE-01 (complemento) -- fecha o vetor secundario registrado no fechamento da REF:
-- create_order() fazia INSERT ... ON CONFLICT (store_id, phone) DO UPDATE SET name = excluded.name
-- SEM verificar auth_user_id do customer ja existente. Qualquer chamador (guest OU autenticado) que
-- soubesse/adivinhasse o telefone de um cliente real ja vinculado a OUTRA conta podia sobrescrever o
-- NOME do cadastro dele (nunca endereco/historico/dado financeiro -- so o campo name).
--
-- Correcao: o UPDATE so sobrescreve o nome quando o customer existente ainda NAO tem auth_user_id
-- (guest/orfao -- comportamento legado 100% preservado, e fecha de brinde o mesmo vetor pelo lado do
-- guest) OU quando auth_user_id ja e' o do proprio chamador (auth.uid(), ele atualizando o proprio
-- nome). Nos demais casos (telefone de OUTRA conta), o pedido continua sendo criado normalmente e
-- vinculado ao customer_id correto -- so o nome deixa de ser sobrescrito.
--
-- Resto do corpo (validacoes, orders/order_items, loyalty, exception handling, resolucao de loja do
-- REF-PROD-GOLIVE-01-fix-tenant-validation.sql) byte-identico ao aplicado em producao agora.
  ...
  exception
    when unique_violation then
      ...
      begin insert into public.application_logs(module,operation,entity,entity_id,request_id,rpc,version,duration_ms,level,message,payload,sqlstate,context,origin)
        values('orders','create_order','order',null,p_request_id,'create_order','prod-golive-01b',v_dur,'error',v_err,v_log,v_state,'unique_violation',current_user);
      exception when others then null; end;
      return jsonb_build_object('ok', false, 'error', v_err, 'sqlstate', v_state);
    when others then
      v_err := sqlerrm; v_state := sqlstate; v_dur := extract(epoch from clock_timestamp()-v_t0)*1000;
      begin insert into public.application_logs(module,operation,entity,entity_id,request_id,rpc,version,duration_ms,level,message,payload,sqlstate,context,origin)
        values('orders','create_order','order',null,p_request_id,'create_order','prod-golive-01b',v_dur,'error',v_err,v_log,v_state,'create_order',current_user);
      exception when others then null; end;
      return jsonb_build_object('ok', false, 'error', v_err, 'sqlstate', v_state);
  end;
end;$function$;

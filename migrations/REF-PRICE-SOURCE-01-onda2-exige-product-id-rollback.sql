-- Rollback REF-PRICE-SOURCE-01 · Onda 2 -- restaura create_order() byte-identico a versao da Onda 1
-- (migrations/REF-PRICE-SOURCE-01-onda1-server-side-pricing.sql), que volta a aceitar item sem
-- product_id confiando no price do client. NAO restaura o comportamento pre-Onda-1 (preco arbitrario
-- para QUALQUER item) -- so desfaz especificamente a exigencia de product_id introduzida na Onda 2.

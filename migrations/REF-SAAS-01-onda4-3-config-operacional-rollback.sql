-- REF-SAAS-01 · Onda 4.3 — Rollback: restaura as 4 chaves na settings GLOBAL (copiando os valores
-- ATUAIS de store_settings da encanto, nao um snapshot congelado — se o admin editou depois da
-- migration, o rollback preserva a edicao), restaura as 8 RPCs + enc_tempo_estimado/
-- enc_enqueue_notification aos corpos originais, remove store_settings.

BEGIN;

-- Copia os valores ATUAIS de volta pra settings (encanto e a unica loja, sempre foi a fonte).
...
WHERE store_id = (SELECT id FROM public.stores WHERE slug = 'encanto')
  AND chave IN ('business_hours_schedule', 'delivery_fee_config', 'delivery_eta_min', 'store_mode')
...
DROP FUNCTION IF EXISTS public.set_store_mode(text, uuid);
...
DROP FUNCTION IF EXISTS public.get_store_mode(uuid);
...
DROP FUNCTION IF EXISTS public.set_delivery_eta(integer, uuid);

CREATE OR REPLACE FUNCTION public.set_delivery_eta(p_min integer)
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public'
...
DROP FUNCTION IF EXISTS public.get_delivery_eta(uuid);
...
DROP FUNCTION IF EXISTS public.set_delivery_fee_config(jsonb, uuid);
...
DROP FUNCTION IF EXISTS public.get_delivery_fee_config(uuid);
...
DROP FUNCTION IF EXISTS public.set_business_hours_schedule(jsonb, uuid);
...
DROP FUNCTION IF EXISTS public.get_business_hours_schedule(uuid);

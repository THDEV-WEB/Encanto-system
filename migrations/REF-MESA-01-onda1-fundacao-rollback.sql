-- ============================================================================
-- REF-MESA-01 · Onda 1 — ROLLBACK
-- Restaura create_order() para a forma anterior (byte-a-byte igual à versão
-- vigente no projeto de E2E antes desta REF, já com o hardening de
-- REF-ADDRESS-GEO-INTEGRITY-01 Onda 2 preservado — esta REF nunca removeu
-- isso, só está desfazendo o que ela mesma acrescentou). Remove
-- get_mesa_config/set_mesa_config, as 3 colunas novas de orders e as chaves
-- mesa_* de store_settings. _resolve_delivery_fee nunca foi tocada por esta
-- REF — nada a reverter nela.
-- ============================================================================

BEGIN;

-- 1) create_order — volta a nao conhecer tipo_pedido/origem_pedido/mesa_identificador.
...
-- 2) Remove get_mesa_config/set_mesa_config (nao existiam antes desta REF).
DROP FUNCTION IF EXISTS public.set_mesa_config(boolean, boolean, boolean, uuid);
DROP FUNCTION IF EXISTS public.get_mesa_config(uuid);

-- 3) Remove as colunas novas de orders (e suas constraints, dropadas junto pelo Postgres).
ALTER TABLE public.orders DROP COLUMN IF EXISTS mesa_identificador;
ALTER TABLE public.orders DROP COLUMN IF EXISTS origem_pedido;
ALTER TABLE public.orders DROP COLUMN IF EXISTS tipo_pedido;

-- 4) Remove as chaves mesa_* de store_settings (nao existiam antes desta REF).
DELETE FROM public.store_settings WHERE chave IN ('mesa_habilitada', 'mesa_canal_qr', 'mesa_canal_admin');

-- Rollback de REF-DELIVERY-FEE-01-step2-orders-schema.sql
-- Restaura create_order e admin_orders_search ao estado ANTERIOR (sem delivery_fee/maquininha_fee) e
-- remove as 2 colunas novas de orders. Aplicar SOMENTE se o Passo 1 (delivery_fee_config) tambem for
-- revertido — o frontend deixa de enviar esses campos assim que a feature for desligada no codigo.
BEGIN;

-- ── 1) admin_orders_search: volta ao RETURNS TABLE anterior (sem as 2 colunas). ──
...
-- ── 2) create_order: volta ao corpo ANTERIOR (sem delivery_fee/maquininha_fee). ──
...
-- ── 3) Remove as colunas (so depois de create_order/admin_orders_search ja nao referenciarem mais). ──
ALTER TABLE public.orders DROP COLUMN IF EXISTS delivery_fee;
ALTER TABLE public.orders DROP COLUMN IF EXISTS maquininha_fee;

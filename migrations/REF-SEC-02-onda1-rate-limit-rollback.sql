-- Rollback de REF-SEC-02-onda1-rate-limit.sql
-- Restaura create_order()/save_structured_address() ao estado exato de antes (sem rate limit),
-- remove o job pg_cron, as funcoes auxiliares e a tabela de contagem.

SELECT cron.unschedule('encanto-rate-limit-purge');
...
DROP FUNCTION IF EXISTS public.rate_limit_purge();
DROP FUNCTION IF EXISTS public._rate_limit_hit(text, int, interval);

...
`<label style="font-size:12px;font-weight:700;color:var(--gray-600);display:block;margin-bottom:6px">CEP</label><input class="addr-search-input" placeholder="00000-000" maxLength="9" value="89120-000"/><div style="margin-top:12px"><div style="background:var(--grape-pale);border-radius:10px;padding:12px 14px;border:1px solid #DDD6FE;margin-bottom:12px"><div style="font-weight:700;font-size:14px;color:var(--amarelo);margin-bottom:4px">✅ CEP encontrado</div><div style="font-size:13px;color:var(--gray-700);line-height:1.7"><b>Rua das Flores</b><br/>Centro · Timbó/SC</div></div><label style="font-size:12px;font-weight:700;color:var(--gray-600);display:block;margin-bottom:4px">Número da residência <span style="color:var(--orange)">*</span></label><input class="addr-search-input" style="margin-bottom:8px" placeholder="Ex: 77" value="77"/><label style="font-size:12px;font-weight:700;color:var(--gray-600);display:block;margin-bottom:4px">Complemento (opcional)</label><input class="addr-search-input" style="margin-bottom:8px" placeholder="Ex: Casa 02, Ap 301" value="Casa 02"/><label style="font-size:12px;font-weight:700;color:var(--gray-600);display:block;margin-bottom:4px">Ponto de referência (opcional)</label><input class="addr-search-input" style="margin-bottom:12px" placeholder="Ex: Perto do mercado" value=""/><button class="addr-confirm-btn">✅ Confirmar endereço</button></div>`;

const GOLDEN_MAP =
`<p style="font-size:12px;color:var(--gray-500);margin-bottom:8px;line-height:1.5">Clique ou arraste o marcador para marcar seu endereço.</p><div class="addr-map-container" style="height:300px"><div style="width:100%;height:100%"></div></div><div style="margin-top:8px;padding:8px 12px;background:var(--grape-pale);border-radius:8px;font-size:13px;color:var(--amarelo);font-weight:600">📍 Rua X, 10</div><div style="margin-top:10px"><label style="font-size:12px;font-weight:700;color:var(--gray-600);display:block;margin-bottom:4px">Número da residência </label><input class="addr-search-input" style="margin-bottom:8px" placeholder="Ex: 77" value=""/><label style="font-size:12px;font-weight:700;color:var(--gray-600);display:block;margin-bottom:4px">Complemento (opcional)</label><input class="addr-search-input" style="margin-bottom:8px" placeholder="Ex: Casa 02, Ap 301" value=""/><label style="font-size:12px;font-weight:700;color:var(--gray-600);display:block;margin-bottom:4px">Ponto de referência (opcional)</label><input class="addr-search-input" style="margin-bottom:12px" placeholder="Ex: Perto do mercado" value=""/><button class="addr-confirm-btn">✅ Confirmar localização no mapa</button></div><p style="font-size:10px;color:var(--gray-400);text-align:center;margin-top:6px">Lat: -26.79500 · Lng: -49.27000</p>`;
...
/* REF-ADDRESS-02 · Onda 5: sugestão escolhida na busca, pendente de número/complemento/referência */
const pickedItemFix = { address: { road: 'Rua João Schlei', suburb: 'Araponguinhas', city: 'Timbó', state: 'Santa Catarina', postcode: '89120-000' }, display_name: 'Rua João Schlei, Araponguinhas, Timbó, SC', lat: '-26.85', lon: '-49.28' };
/* Onda 5b: sugestão de confiança baixa (achado do ADR §0.2 — rua casada, número não confirmado) */
const pickedItemBaixaConfianca = { ...pickedItemFix, _confidence: 'street_level' };
const GOLDEN_SEARCH_PICKED =
`<div style="padding:8px 12px;background:var(--grape-pale);border-radius:8px;font-size:13px;color:var(--amarelo);font-weight:600;margin-bottom:8px">📍 Rua João Schlei</div><button type="button" style="background:none;border:none;color:var(--grape);font-size:12px;font-weight:700;cursor:pointer;padding:0;margin-bottom:10px;font-family:var(--font-body)">← Escolher outro endereço</button><label style="font-size:12px;font-weight:700;color:var(--gray-600);display:block;margin-bottom:4px">Número da residência <span style="color:var(--orange)">*</span></label><input class="addr-search-input" style="margin-bottom:8px" placeholder="Ex: 77" value="123"/><label style="font-size:12px;font-weight:700;color:var(--gray-600);display:block;margin-bottom:4px">Complemento (opcional)</label><input class="addr-search-input" style="margin-bottom:8px" placeholder="Ex: Casa 02, Ap 301" value=""/><label style="font-size:12px;font-weight:700;color:var(--gray-600);display:block;margin-bottom:4px">Ponto de referência (opcional)</label><input class="addr-search-input" style="margin-bottom:12px" placeholder="Ex: Perto do mercado" value=""/><button class="addr-confirm-btn">✅ Confirmar endereço</button>`;

const GOLDEN_SEARCH_ERRO =
`<input class="addr-search-input" placeholder="Rua, número, bairro ou local..." value=""/><button class="addr-gps-btn"><span>🎯</span> Usar minha localização atual</button><div style="padding:8px 10px;background:#FEF3C7;border:1px solid #FDE68A;border-radius:8px;font-size:12px;color:#92400E;line-height:1.5;margin-bottom:10px;margin-top:10px">⚠️ Sem permissão de localização. Ative nas configurações do navegador, ou busque pelo endereço/CEP.</div><div style="margin-top:12px"><div class="addr-section-label">Dicas de busca</div><div style="font-size:12px;color:var(--gray-500);line-height:1.8;padding:4px 0">• Ex: <b>Rua das Flores, 123</b><br/>• Ex: <b>João Schlay 77</b><br/>• Ex: <b>Nome do bairro ou ponto de referência</b></div></div>`;

const GOLDEN_SEARCH_PICKED_BAIXA_CONFIANCA =
`<div style="padding:8px 12px;background:var(--grape-pale);border-radius:8px;font-size:13px;color:var(--amarelo);font-weight:600;margin-bottom:8px">📍 Rua João Schlei</div><button type="button" style="background:none;border:none;color:var(--grape);font-size:12px;font-weight:700;cursor:pointer;padding:0;margin-bottom:10px;font-family:var(--font-body)">← Escolher outro endereço</button><div style="padding:8px 10px;background:#FEF3C7;border:1px solid #FDE68A;border-radius:8px;font-size:12px;color:#92400E;line-height:1.5;margin-bottom:10px">⚠️ Encontramos a rua, mas não confirmamos o número exato — confira o número abaixo.</div><label style="font-size:12px;font-weight:700;color:var(--gray-600);display:block;margin-bottom:4px">Número da residência <span style="color:var(--orange)">*</span></label><input class="addr-search-input" style="margin-bottom:8px" placeholder="Ex: 77" value=""/><label style="font-size:12px;font-weight:700;color:var(--gray-600);display:block;margin-bottom:4px">Complemento (opcional)</label><input class="addr-search-input" style="margin-bottom:8px" placeholder="Ex: Casa 02, Ap 301" value=""/><label style="font-size:12px;font-weight:700;color:var(--gray-600);display:block;margin-bottom:4px">Ponto de referência (opcional)</label><input class="addr-search-input" style="margin-bottom:12px" placeholder="Ex: Perto do mercado" value=""/><button class="addr-confirm-btn">✅ Confirmar endereço</button>`;
